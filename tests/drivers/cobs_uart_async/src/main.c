...
/* Server thread with very large stack to debug overflow issue */
K_THREAD_DEFINE(server_tid, 32768, server_thread, NULL, NULL, NULL, 7, 0, 0);

/* TX thread - continuously sends packets */
static void client_tx_thread(void *p1, void *p2, void *p3)
{
	...
	for (uint32_t seq = 0; seq < TEST_PACKET_COUNT; seq++) {
		...
		/* Small yield to allow RX thread to run if needed */
		k_yield();
	}
